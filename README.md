...
**엔딩 아이디어**  
1. 탈출하여 도망갔다. 하지만 나가는 도중, 조금씩 우는 건지 소리지르는 건지 이명이 들리며 찜찜하다.  
2. 한을 풀어주고 탈출하였다. 한을 풀어주는 방법을 찾아 실행하면 다음 맵에서  
다른 버전의 첫 가든 맵이 나오며 작은 선물을 주고 사용하면 도서관으로 돌아간다.
도서관을 나가면 음악과 함께 크레딧이 나온다. [열린 엔딩]  
  
**플레이 방식**  
- 목표: **무한히 이어지는 공간** 속에서 출구를 찾아 이동  
- 퍼즐 요소  
  - 버튼 누르기, 레버 조작, 열쇠 획득  
  - 힌트를 활용해 해답 도출  
  (힌트 형태: 백룸 물 담는 것처럼, 포스트잇 그림 붙여두기)  
- 위험 요소  
  - 몬스터는 **특정 구간마다 다른 몹 출현**  
  - 퍼즐 실패 시 위협 기믹 발동  
  - **점프스퀘어로 갑작스럽게 등장**하여 공포 유도
